 ...
 * bound, meaning it gets a value, when invoking the {@link Carrier#run}
 * or {@link Carrier#call} method of an {@link
 * ExtentLocal.Carrier}. {@link Carrier} instances are generated by the static method
 * {@link #where(ExtentLocal, Object)}.
 * Code executed by the {@link run()} or {@code call()}  operations will use the {@link #get()} method to
 * get the value of a bound extent local. The extent-local variable reverts to
 * being unbound (or its previous value) when the  {@code run()} or {@code call()} method that bount it exits.
 *
 ...
 * local already has a value, then {@link Carrier#run} or {@link Carrier#call}
 * may be invoked to run another operation with a newly-bound
 * value. Code executed by {@code run()} or {@code call()} then "sees" the new value of the
 * extent local. The extent local reverts to its previous value when
 * the {@link Carrier#run()} or {@link Carrier#call()} method exits.
 *
 ...
 * thread. The values of bound extent-local variables are inherited by
 * threads launched by a {@link StructuredTaskScope}. Any extent-local
 * bindings present in the thread that calls {@link StructuredTaskScope#fork(Callable)}
 * will be visible to the forked thread. This means that you may bind
 * extent-local variables in an outer context then use a {@link StructuredTaskScope}
 * to start many threads, and those threads may
 * use {@link #get} to get the values of extent-local variables bound by the
 * thread starting the {@link StructuredTaskScope}.
 *
 * @implNote

 * Extent-local variables are designed to be used in fairly small
 * numbers. {@link #get} initially performs a linear search through
 * enclosing scopes to find an extent-local variable's innermost binding. It
 ...
 * extent-local variables that it uses cyclically, the cache hit rate will be
 * low and performance will be poor. This design allows extent-local
 * inheritance by {@link StructuredTaskScope} threads to be very fast:
 * in essence, no more than copying a pointer, and leaving a
 * extent-local binding also requires little more than updating a
 * pointer.
 *
...
public final class ExtentLocal<T> {
     ...
     * When {@link #run} or {@link #call} is invoked, the extent-local variable mappings in this set
     * are bound, such that calling the {@link ExtentLocal.get} method returns the associated
     * value.
     * @since 19
     */
    public static final class Carrier {
        ...
        /**
         * Runs a value-returning operation with some extent-local variables bound to values.
